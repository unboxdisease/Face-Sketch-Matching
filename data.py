...
class DatasetInference(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.in_channels == 3:
            data = Image.open(img_path).convert('RGB').resize((self.resolution, self.resolution))
        else:
            data = Image.open(img_path).convert('L').resize((self.resolution, self.resolution))

        data = np.array(data)
        data = ((data[:,:,::-1] / 255.) - 0.5) / 0.5
        data = torch.tensor(data.transpose(2,0,1)).float()
